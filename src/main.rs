...
use crossbeam::{select, tick, unbounded, Receiver};
...
fn setup_ui_events() -> Receiver<InputEvent> {
    ...
    thread::spawn(move || {
        ...
        loop {
            ui_events_sender.send(reader.next().unwrap()).unwrap(); // TODO: unwrap
        }
    });
    ui_events_receiver
}
...
fn setup_logfile(logfile_path: &Path) {
    ...
    fern::Dispatch::new()
        ...
        .apply()
        .unwrap(); // TODO: unwrap
    let _stderr_redirect = Redirect::stderr(logfile).unwrap(); // TODO: unwrap
}
    ...
    match colorscheme {
        ...
        _ => {
            let json_string = match colorscheme {
                ...
                args::Colorscheme::Solarized => include_str!("../colorschemes/solarized.json"),
...
fn setup_widgets(args: &Args, colorscheme: &colorscheme::Colorscheme) -> Widgets {
    ...
    let mem_widget = widgets::MemWidget::new();
...
fn update_widgets(widgets: &mut Widgets) {
